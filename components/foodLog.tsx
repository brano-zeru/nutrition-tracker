...
export function FoodLog() {
  const { getCurrentDayLog, addFoodEntry, removeFoodEntry, updateFoodEntry } = useNutrition()
  const [isAddOpen, setIsAddOpen] = useState(false)
  const [duplicateEntry, setDuplicateEntry] = useState<FoodEntry | null>(null)
  ...
  const confirmDuplicate = () => {
    if (duplicateEntry) {
      ...
      addFoodEntry({
        ...
        protein: Math.round(duplicateEntry.protein * multiplier * 10) / 10,
        notes: multiplier !== 1 ? `${multiplier}x portion` : duplicateEntry.notes,
      })
  ...
  return (
    <Card className="border-border/50 bg-card">
      <CardHeader className="flex flex-row items-center justify-between pb-3 sm:pb-4 px-3 sm:px-6">
        <CardTitle className="text-base sm:text-lg font-medium">Food Log</CardTitle>
        <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
          <DialogTrigger asChild>
            <Button size="sm" className="bg-primary text-primary-foreground hover:bg-primary/90 text-xs sm:text-sm h-8 sm:h-9">
      ...
      <CardContent className="px-3 sm:px-6">
        {entries.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 sm:py-12 text-center">
            ...
            <p className="text-muted-foreground text-xs sm:text-sm">No food entries yet</p>
              ...
              {entries.map((entry) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    <span className="text-calories font-mono">{entry.calories} cal</span>
                    <span className="text-protein font-mono">{entry.protein}g protein</span>
                  </div>
                  {entry.notes && (
                    <p className="text-[10px] text-muted-foreground mt-1.5">{entry.notes}</p>
                  )}
                </div>
              ))}
            </div>
      ...
      <Dialog open={!!duplicateEntry} onOpenChange={() => setDuplicateEntry(null)}>
        <DialogContent className="bg-card border-border max-w-[calc(100%-2rem)] sm:max-w-md mx-auto">
          ...
          <FieldGroup className="gap-4">
            <Field>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Use 1 for same amount, 2 for double, 0.5 for half, etc.
              </p>
            </Field>
              ...
              <div className="bg-secondary/50 rounded-lg p-4">
                ...
                <div className="flex justify-between">
                  <span className="text-calories font-medium">
                    {Math.round(duplicateEntry.calories * (parseFloat(duplicateMultiplier) || 1))} cal
                  </span>
                  <span className="text-protein font-medium">
                    {Math.round(duplicateEntry.protein * (parseFloat(duplicateMultiplier) || 1) * 10) / 10}g protein
                  </span>
                </div>
              </div>
            )}
          ...
          <DialogFooter className="mt-4">
            ...
            <Button onClick={confirmDuplicate} className="bg-primary text-primary-foreground">
              Add Entry
            </Button>
          </DialogFooter>
        </DialogContent>
