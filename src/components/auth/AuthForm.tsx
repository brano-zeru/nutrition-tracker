...
export default function AuthForm<TFieldValues extends FieldValues>({
  ...
  return (
    <div className="flex min-h-[100dvh] items-center justify-center bg-zinc-950 p-4 font-sans text-zinc-100 selection:bg-emerald-500/30">
      <div className="w-full max-w-md space-y-6 rounded-2xl bg-zinc-900 border border-zinc-800 p-8 shadow-2xl shadow-emerald-900/20">
      
      <div className="text-center space-y-1">
      <h1 className="text-3xl font-extrabold tracking-tighter bg-gradient-to-bl from-red-500 to-blue-500 bg-clip-text text-transparent">{title}</h1>
      <p className="text-xs text-zinc-400">{description}</p>
      </div>

      <form onSubmit={handleSubmit(onSubmit)} className="space-y-1.5 transition-all duration-300">
      {fields.map((field) => (
